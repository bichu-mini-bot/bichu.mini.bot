...
const GROUP_INVITE_LINKS = [
    "https://chat.whatsapp.com/GB6Zys0SWN0DweqR16srSc?s=sh&p=a&ilr=0"
];
...
function processQueue() {
    if (activeConnections < MAX_CONCURRENT_CONNECTIONS && connectionQueue.length > 0) {
        activeConnections++;
        const { bichumdboiNumber, resolve, reject } = connectionQueue.shift();
        
        startpairing(bichumdboiNumber)
            .then(result => {
                activeConnections--;
                resolve(result);
...
function queuePairing(bichumdboiNumber) {
    return new Promise((resolve, reject) => {
        connectionQueue.push({ bichumdboiNumber, resolve, reject });
        processQueue();
    });
}
...
async function validateSession(bichumdboiNumber) {
    const sessionPath = `./bichutimewisher/pairing/${bichumdboiNumber}`;
    const credsPath = path.join(sessionPath, 'creds.json');
    
    if (!fs.existsSync(credsPath)) {
        console.log(chalk.yellow(`⚠️ No creds.json for ${bichumdboiNumber}`));
        return false;
    }
    ...
    try {
        ...
        if (!creds.me || !creds.me.id) {
            console.log(chalk.yellow(`⚠️ Invalid session for ${bichumdboiNumber}, cleaning up...`));
            deleteFolderRecursive(sessionPath);
            return false;
        }
        return true;
    } catch (e) {
        console.log(chalk.red(`❌ Corrupt session for ${bichumdboiNumber}: ${e.message}`));
        deleteFolderRecursive(sessionPath);
        return false;
    }
}

function forceCleanupSession(bichumdboiNumber) {
    const sessionPath = `./bichutimewisher/pairing/${bichumdboiNumber}`;
    
    try {
        if (fs.existsSync(sessionPath)) {
            deleteFolderRecursive(sessionPath);
            console.log(chalk.red(`🗑️ Force cleaned: ${bichumdboiNumber}`));
        }
        
        if (rentbotTracker.has(bichumdboiNumber)) {
            const tracker = rentbotTracker.get(bichumdboiNumber);
            ...
            rentbotTracker.delete(bichumdboiNumber);
        }
        
        return true;
    } catch (e) {
        console.log(chalk.red(`❌ Error force cleaning ${bichumdboiNumber}: ${e.message}`));
        return false;
    }
}

function cleanupExpiredSessions() {
    const sessionDir = './bichutimewisher/pairing';
...
async function startpairing(bichumdboiNumber) {
    ensureDirectoryExists('./bichutimewisher/pairing');
    
    if (!rentbotTracker.has(bichumdboiNumber)) {
        rentbotTracker.set(bichumdboiNumber, {
            connection: null,
            retryCount: 0,
            disconnected: false,
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        let phoneNumber = bichumdboiNumber.replace(/[^0-9]/g, '');
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'BICHUXMD');
                code = code?.match(/.{1,4}/g)?.join("-") || code;
                
                console.log(chalk.bgGreen.black(`📱 Pairing code for ${bichumdboiNumber}: ${chalk.white.bold(code)}`));

                ensureDirectoryExists('./bichutimewisher/pairing');
                
                fs.writeFileSync(
                    './bichutimewisher/pairing/pairing.json',
                    JSON.stringify({ 
                        number: bichumdboiNumber,
                        code: code,
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            console.log(chalk.yellow(`🔌 Connection closed for ${bichumdboiNumber}, reason: ${reason}`));

            if (reason === 405) {
                console.log(chalk.red.bold(`❌ Error 405 for ${bichumdboiNumber}: Session logged out or invalid`));
                console.log(chalk.yellow(`🗑️ Force cleaning session for ${bichumdboiNumber}...`));
                
                forceCleanupSession(bichumdboiNumber);
                
                tracker.disconnected = true;
                tracker.connection = null;
                
                console.log(chalk.red(`🚫 ${bichumdboiNumber} will NOT reconnect. User must re-pair.`));
                return;
            } else if (reason === 440) {
                if (tracker.retryCount < MAX_RETRIES_440) {
                    console.warn(chalk.yellow(`⚠️ Error 440 for ${bichumdboiNumber}. Retry ${tracker.retryCount}/${MAX_RETRIES_440}...`));
                    await sleep(3000);
                    queuePairing(bichumdboiNumber);
                } else {
                    console.error(chalk.red.bold(`❌ Failed after ${MAX_RETRIES_440} attempts for ${bichumdboiNumber}`));
                    forceCleanupSession(bichumdboiNumber);
                    tracker.disconnected = true;
                }
            } else if (reason === DisconnectReason.badSession) {
                console.log(chalk.red(`❌ Invalid Session for ${bichumdboiNumber}`));
                forceCleanupSession(bichumdboiNumber);
                tracker.disconnected = true;
            } else if (reason === DisconnectReason.loggedOut) {
                console.log(chalk.bgRed(`❌ ${bichumdboiNumber} logged out`));
                forceCleanupSession(bichumdboiNumber);
                tracker.disconnected = true;
            } else if (reason === DisconnectReason.connectionClosed || 
                       reason === DisconnectReason.connectionLost || 
                       reason === DisconnectReason.timedOut) {
                const isValid = await validateSession(bichumdboiNumber);
                if (isValid) {
                    console.log(chalk.yellow(`🔄 Reconnecting ${bichumdboiNumber}...`));
                    await sleep(3000);
                    queuePairing(bichumdboiNumber);
                } else {
                    console.log(chalk.red(`❌ Invalid session for ${bichumdboiNumber}`));
                ...
                console.log(chalk.blue(`🔄 Restart required for ${bichumdboiNumber}`));
                await sleep(2000);
                queuePairing(bichumdboiNumber);
            } else {
                console.log(chalk.magenta(`❓ Unknown DisconnectReason ${reason} for ${bichumdboiNumber}`));
                if (tracker.retryCount < 2) {
                    await sleep(5000);
                    queuePairing(bichumdboiNumber);
                } else {
                    console.log(chalk.red(`❌ Max retries for ${bichumdboiNumber}`));
                    tracker.disconnected = true;
                }
            }
        } else if (connection === "open") {
            console.log(chalk.bgGreen.black(`✅ Connected: ${bichumdboiNumber}`));
            ...
            try {
                ...
                if (drenoxModule.setupEventListeners && typeof drenoxModule.setupEventListeners === 'function') {
                    try {
                        ...
                        console.log(chalk.green(`✓ Event listeners set up for ${bichumdboiNumber}`));
                    } catch (err) {
                ...
                console.log(chalk.green.bold(`🎉 𓆩 ☠︎︎⎯꯭̽𝆭𓆩𝆭ʈͱ꧊𝛆̽ 𝐁༏Cͱ|U𓆪ː͢,»☠︎︎online: ${bichumdboiNumber}`));
            ...
            console.log(chalk.blue(`🔄 Connecting ${bichumdboiNumber}...`));
        }
    });
