...
const adminFilePath = path.join(__dirname, 'bichutimewisher', 'admin.json');
let adminIDs = [];
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendPhoto(
    ...
    {
      caption: `🪀 *⎯꯭̽𝆭𓆩𝆭ʈͱ꧊𝛆̽ 𝐁༏Cͱ|U𓆪ː͢,» 𝑴𝑫💀*\n\n╔════════════════════╗\n ⤷ /pair <wa_number>\n ⤷ /unpair <wa_number>\n╚════════════════════╝`,
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: "👑 Owner", url: "https://t.me/bichuxboy" }]
        ]
      }
    }
  );
...
bot.onText(/\/pair(?:\s+(.+))?/, async (msg, match) => {
  ...
  const pairingFolder = path.join(__dirname, 'bichutimewisher', 'pairing');
...
bot.onText(/\/unpair(?:\s+(.+))?/, async (msg, match) => {
  ...
  try {
    ...
    const pairingPath = path.join(__dirname, 'bichutimewisher', 'pairing');
...
(async () => {
  ...
  console.log('✅ Bot Username: @bichu_mdbot');
